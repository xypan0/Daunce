...
def model_output_f(logits, labels):
    ...
    cret = nn.CrossEntropyLoss(reduction='none')
    return cret(logits, labels)
...
if __name__ == '__main__':
    ...
    def train(epoch):
        ...
        for batch_idx, ((inputs, targets), indexes) in enumerate(tqdm(trainloader)):
            ...
            loss = args.gamma * (model_loss-mean_base_loss) - fo
            loss.backward()
    ...
    for epoch in range(1, args.epoch+1):
        res=train(epoch)
        test(epoch)
        grad_info_all.extend(res)
    json.dump(grad_info, open('grad_info.json', 'w'), ensure_ascii=False, indent=2)
